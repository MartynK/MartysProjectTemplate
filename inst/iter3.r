# Runtime still unacceptable despite running it on desolve
...
NROW <- 1000
n_perpar <- floor((NROW / length(unique(Theoph2$Time)))^(1/2))

par_mat <- expand.grid( time = unique(Theoph2$Time),
                        q_plas = NA,
                        q_gi = NA,
                        k_a  = seq(0.001,2,length.out=n_perpar),
                        k_e  = seq(0.001,2,length.out=n_perpar)
                         )
par_levels <- par_mat %>% select(!(time)) %>% distinct()

pb <- txtProgressBar()
for (i in 1:nrow(par_levels)) {

    initial_conditions <- c(q_gi  = 100,
                            q_plas = 0)

    parameters <- list(par_ka = par_levels$k_a[i],
                       par_ke = par_levels$k_e[i])

    result <- deSolve::ode(y = initial_conditions,
                           times = sort(unique(c(0,unique(Theoph2$Time)))),
                           func = model_for_ode,
                           parms = parameters,
                           method = "adams",
    ...
    result_df <- as.data.frame(result) %>%
                   mutate( k_a = parameters$par_ka,
                           k_e = parameters$par_ke)

    par_mat[((i-1)*78+1):(i*78),] <- result_df
    setTxtProgressBar(pb, i/nrow(par_levels))
}
close(pb)

interpn::interpn(outmode=3, par_mat$q_plas,
                 par_mat$time, par_mat$k_a, par_mat$k_e,
